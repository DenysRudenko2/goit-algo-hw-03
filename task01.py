...
DEFAULT_PATH_FROM = 'Source'
DEFAULT_PATH_TO = 'Destination'
ds = os.sep
...
def copy_files(dir_from: str, destination_dir: str):
    for item in os.listdir(dir_from):
        if os.path.isdir(os.path.join(dir_from, item)):
            ...
            extension = UNKNOWN_EXTENSION if len(extension) in [0, 1] else extension[1:]
...
if __name__ == '__main__':
    arguments = sys.argv
    path_from = arguments[1] if len(arguments) > 1 else DEFAULT_PATH_FROM
    path_to = arguments[2] if len(arguments) > 2 else DEFAULT_PATH_TO
